...
const TechStack = () => {
  ...
  const [defdata, defsetData] = useState(["JavaScript", "React", "Node.js", "Express", "MongoDB", "HTML", "CSS"]);
  ...
  return (
    <div className='my-2 w-full md:w-96'>
      ...
      <div className="h-32 overflow-hidden border rounded-b bg-white shadow-lg">
        {userIDFE && (<div className="animate-verticalScroll flex flex-col gap-2 items-center">
          {/* Repeat stack twice for infinite scroll illusion */}
          {data.map((item) => (
            <div key={item._id} className="text-sm font-medium text-gray-800  bg-blue-300 border rounded min-w-min py-1 px-1  hover:bg-blue-700">
              {item.teckName}
            </div>
        ...
        {!userIDFE && (
          <div className="animate-verticalScroll flex flex-col gap-2 items-center">
            {/* Repeat stack twice for infinite scroll illusion */}
            {defdata.map((item) => (
              <div key={item} className="text-sm font-medium text-gray-800  bg-blue-300 border rounded min-w-min py-1 px-1  hover:bg-blue-700">
                {item}
              </div>
